...
//        Timber.d("Loading AppOpenAd")
       AppOpenAd.load(
            context,
            if (BuildConfig.USE_LOCAL_ADMOB_IDS) BuildConfig.ADMOB_APP_OPEN_KEY else PrefsHelper.getAdmobAppOpenId(),
            getAdRequest(),
...
//            Timber.d("Can not show ad. Probably ad not available")
            retryLoadingAd(AdType.APP_OPEN, activity)
        }
    }
...
//        Timber.d("Start loading InterstitialAd")
        InterstitialAd.load(
            activity,
            if (BuildConfig.USE_LOCAL_ADMOB_IDS) BuildConfig.ADMOB_INTERSTITIAL_KEY else PrefsHelper.getAdmobInterId(),
            getAdRequest(),
...
//        Timber.d("Start loading MoreTimeInterstitialAd")
        InterstitialAd.load(
            activity,
            if (BuildConfig.USE_LOCAL_ADMOB_IDS) BuildConfig.ADMOB_MORE_TIME_INTERSTITIAL_KEY else PrefsHelper.getAdmobRewardedInterId(),
            getAdRequest(),
            object : InterstitialAdLoadCallback() {
                override fun onAdLoaded(ad: InterstitialAd) {
...
//        Timber.d("Start loading RewardedInterstitialAd")
        RewardedInterstitialAd.load(
            activity,
            if (BuildConfig.USE_LOCAL_ADMOB_IDS) BuildConfig.ADMOB_REWARDED_INTERSTITIAL_KEY
...
//        Timber.d("Start loading NativeAd")
        AdLoader.Builder(
            activity,
            if (BuildConfig.USE_LOCAL_ADMOB_IDS) BuildConfig.ADMOB_NATIVE_KEY else PrefsHelper.getAdmobNativeId()
        )
            .forNativeAd {
                nativeAd = it
...
//            Timber.d("Won't load BannerAd")
        ...
        bannerAd?.adUnitId =
            if (BuildConfig.USE_LOCAL_ADMOB_IDS) BuildConfig.ADMOB_BANNER_KEY else PrefsHelper.getAdmobBannerId()
        bannerAd?.setAdSize(AdSize.getCurrentOrientationAnchoredAdaptiveBannerAdSize(activity, adWidth))
